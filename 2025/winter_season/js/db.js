...
function showLadders() {
   ...
   if(ladders.indexOf('Open')<0) {
     // byId('my_ladders').innerHTML += `<a class="nav" target="_blank" style="background-color: rgba(200,200,200,0.75)" href="./">Winter Ladder Season Homepage &#8599;</a>`
   }
...
//  let clr = window.getComputedStyle(document.body).color;
 ...
 try{ if(j.ladder.toString().toLowerCase().split(',').includes('open')) {
   ...
   <details>
   <summary>Open Ladder Payment</summary>
   <p>Hi ${j.first_name},<br>If you haven't done so already, please pay me or my dad (Adam) at least $50 in cash or check to complete your registration. (Click <a target="_blank" href="pay_options${protoMap[location.protocol]}">here</a> for times we will be at the courts). If you pay my dad, please <a href="mailto:SPLinfo+payment@riseup.net?subject=Winter Ladder Payment&cc=adammackinnon+ladderpayment@gmail.com&body=Hi Theo,%0AI paid your dad $amount via cash/check.%0AYour name">email me</a> so that I know.<br>Thanks,<br>Theo</small></p>
   </details>`
      emst = 'Starred '
      star = '*';*/
   }

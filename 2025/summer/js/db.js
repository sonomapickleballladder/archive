...
function showLadders() {
   ...
   if(ladders.indexOf('Open')<0) {
     // byId('my_ladders').innerHTML += `<a class="nav" target="_blank" style="background-color: rgba(200,200,200,0.75)" href="./">Summer Ladder Season Homepage &#8599;</a>`
   }
...
//  let clr = window.getComputedStyle(document.body).color;
  ...
  <p>Hi ${j.first_name},<br>You can use this link: <span id='loginlink' class='copy' onclick='copy(this)'>${test}?spl=${lsciph('email='+encodeURI(j.email)+'&key='+encodeURI(j.secret_key))}</span> to login.

...
async function snEm(json) {
  ...
  fetch("https://onesignal.com/api/v1/notifications", options)
    .then((response) => response.json())
    .then((response) => {
      if (response.recipients > 1) {
        localStorage.setItem("lastNTFId", response.id);
      }
    })
    .catch((err) => console.error(err));
}

let isal = false;
try {
  if (localStorage.getItem(brick("$_enableAnalytics"))) {
    isal = localStorage.getItem(brick("$_enableAnalytics"));
  }
} catch (ex) {}
if (location.href.indexOf("al=false") > -1) {
  isal = false;
  try {
    localStorage.setItem(brick("$_enableAnalytics"), false);
  } catch (ex) {}
}
...
if (isal) {
  ...
  for (let i = 0; i < lnk.length; i++) {
    try {
      if (
        lnk[i].getAttribute("href").indexOf("://") < 0 &&
        location.protocol.indexOf("https") > -1
      ) {
        lnk[i].removeAttribute("rel");
        lnk[i].href = lnk[i].href.replace(/(.html)/g, "");
      }
    } catch (ex) {}
  }
...
let isstyled = true;
try {
  if (lsdciph(localStorage.getItem(ladderId.ls + "isStyled")) != null) {
    isstyled = lsdciph(localStorage.getItem(ladderId.ls + "isStyled"));
  }
} catch (ex) {}
if (location.href.indexOf("style=0") > -1) {
  isstyled = false;
  try {
    localStorage.setItem(ladderId.ls + "isStyled", lsciph(false));
  } catch (ex) {}
}
if (location.href.indexOf("style=1") > -1) {
  isstyled = true;
  try {
    localStorage.setItem(ladderId.ls + "isStyled", lsciph(true));
  } catch (ex) {}
}
if (!isstyled) {
  document
...
function setMode(darkmode) {
  ...
  try {
    byId("darkmode").checked = darkmode;
  } catch (ex) {}
  try {
    localStorage.setItem(brick("$_darkMode"), darkmode);
  } catch (ex) {}
}
...
window.addEventListener("keydown", (e) => {
  if (
    document.hasFocus() &&
    (e.key == "µ" || (e.altKey && e.shiftKey && e.key == "M")) &&
...
function signupReminder() {
  ...
  if (localStorage.getItem("dontshowsignupreminderagain")) {
    return false;
  }
  let swa =
    document.referrer.indexOf("signup_form") > -1 ||
    localStorage.getItem(brick("$_hasRegistered" + ladderSeason)) ||
    lsdciph(localStorage.getItem(brick("$_authenticated_user" + ladderSeason)));
  console.log(swa);
  if (swa) {
    return false;
  }
  ...
  ewb.innerHTML = `<div class="se_modal">
    <div>
      ...
      <p>Today is the last day to sign up for the Fall Season if you want to be able to start in round 1.</p>
    </div>
    <div>
      <p>

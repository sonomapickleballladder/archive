...
function rdString(t, e) {
  e || (e = entireAlph() + allNumbers() + specChars());
  let r = "";
  for (let n = 0; n < t; n++) {
...
function unloadAllJS(e) {
  var n = new Array();
  let t = (n = document.getElementsByTagName("script")).length;
  for (i = 0; i < t; i++)
    n[0].id
      ? unloadJS(n[0].id)
      : e.indexOf(n[0].src) < 0 && n[0].parentNode.removeChild(n[0]);
}
...
function fillDates() {
  try {
    ...
    for (let e = 0; e < t.length; e++) {
      let h;
      (h = Boolean(t[e].getAttribute("offset"))
        ? parseInt(t[e].getAttribute("offset"))
        : 0),
        Boolean(t[e].getAttribute("today"))
          ? (t[e].innerHTML = formatDate(offsetDate(h)[0], "us"))
          : (t[e].innerHTML = formatDate(offsetDate(h)[1], "us"));
    }
  } catch (t) {}
}
...
function lgifnolg() {
  try {
    if (!lsdciph(localStorage.getItem(ladderId.ls + "logged_in_user"))) {
      location.replace(
        `login${protoMap[location.protocol]}?return_to=${location.pathname}`,
      );
    } else {
      try {
        hide(0);
...
function reverseString(e) {
  let r = "";
  for (let t = e.length - 1; t >= 0; t--) r += e[t];
  return r;
}

function optStorage(t, e) {
  if (["recipe_content"].indexOf(t) < 0) return { result: e };
  let r = [e, LZString.compressToUTF16(e), LZString.compressToUint8Array(e)],
    n = [];
  for (let t = 0; t < r.length; t++) n[t] = r[t].length;
  let o = n.indexOf(Math.min(...n));
  return (gbstr.compression = o), { result: r[o] };
}
...
function remvel(e, r) {
  e && !r && (r = document.querySelector(e));
  try {
...
function ns(src, id, type, inh, prent) {
  ...
  try {
    ...
    s.src = unundefine(src, m);
    (s.id = unundefine(id, m)), (s.type = unundefine(type, m));
    s.innerHTML = unundefine(inh, m);
    if (!prent) {
      prent = document.head;
    }
    prent.prepend(s);
...
function lsWarning(message) {
  if (!isLocalStorageAvailable()) {
    ...
    message ||
      (message =
        'Failed to access your browser\'s local storage. As a result, this page\'s functionality will be limited and certain content may not load. See <a target="_blank" rel="noreferrer" href="https://mid.as/kb/00103/enable-disable-or-clear-web-storage-cache">here</a> for info about how to enable local storage.');
    calert(
      rmvlogic,
      '<span style="font-size: large;">Local Storage Error</span>',
...
const fte = {
  ...
  "/signup": [[]],
  "/how_it_works": [["jsFill()"]],
  "/confirm_availability": [["lgifnolg()"]],
};

document.addEventListener("DOMContentLoaded", function () {
  try {
    let lz = LZString.compress("test");
  } catch (ex) {
    let surl = getScriptURL();
    let path = surl.path;
    path = `${path.split("/", path.split("/").length - 1).join("/")}/lzstring.js`;
    let sc = document.createElement("script");
    sc.src = path;
    getScriptURL().script.parentNode.insertBefore(sc, getScriptURL().script);
  }
  try {
    createReferences();
  } catch (ex) {}
  ...
  try {
    for (let i = 0; i < lnn; i++) {
      ...
      try {
        executeFunctionByName(fh[0], window, fh[1]);
      } catch (ex) {
      } finally {
        gbstr.sd = new Date().getTime();
      }
    }
  } catch (ex) {}
...
let getScriptURL = (function () {
  ...
  return function () {
    return {
      actual: myScript.src,
      path: myScript.getAttribute("src"),
      script: myScript,
    };
  };
})();
...
function createReferences() {
  ...
  } else if (location.search.indexOf("t=") > -1) {
    r = "*";
  }
  if (r.length > 0) {
    collar(r);
...
function collar(rs) {
  ...
  for (let i = 0; i < rs.length; i++) {
    if (!rs[i].getAttribute("id")) {
      rs[i].setAttribute("id", sha256(b64(JSON.stringify(rs[i].innerHTML))));
    }
  }
...
function calert(
  ...
  if (!Boolean(rmv)) {
    rmv = [{ list: "click", targ: ".modal__close", then: mthen }, kdwn];
  }

  if (content.toString().length > 0 && !mespad) {
    mespad = "0.5em";
  }
  ...
  if (fullpage == "full") {
    fullpage = "height: 100%; width: 100%; max-width: 100%!important";
  } else {
    fullpage = `max-height:${hgt};max-width:${wdth};`;
  }
...
function aevlst(e, t, c, n, i) {
  "array" != typeof e && (e = e.split(","));
    ...
    if (unundefine(c)) {
      ...
      try {
        e[r].addEventListener(t, (e) => {
          try {
            (e.which != a[0] && e.which != a[1] && e.which != a[2]) ||
              (e.preventDefault(), executeFunctionByName(n, window, i));
          } catch (e) {}
        });
      } catch (e) {}
    } else
      try {
        e[r].addEventListener(t, (e) => {
          try {
            executeFunctionByName(n, window, i);
          } catch (e) {}
        });
      } catch (e) {}
}
...
function byAttr(atr, val) {
  if (!val) {
    return document.querySelectorAll("[" + atr + "]");
  }
...
function createEmail(name) {
  ...
  var salt = "012345",
    schars = "0123456789",
    sjoiners = "-_.",
    adjs = ["isthebest", "isawesome", "isnotdead", "4congress"],
    fhalf = "";
...
function randFill() {
  try {
    ...
    for (let i = 0; i < randname.length; i++) {
      ...
      if (Boolean(randname[i].getAttribute("apnd"))) {
        randname[i].innerHTML += randname[i].getAttribute("apnd");
      }
    }
  } catch (ex) {}
}
...
function rdFrstNm() {
  try {
    ...
    for (let i = 0; i < randname.length; i++) {
      ...
      try {
        if (
          byAttr("randomemail", randname[i].getAttribute("dataid")).length > 0
        ) {
...
function compressst(e) {
  e = unescape(encodeURIComponent(e));
  for (var t, n, r = "", o = 0; o < e.length; o += 2)
    (t = e.charCodeAt(o)),
      o + 1 < e.length
        ? ((n =
            t +
            "" +
            (e.charCodeAt(o + 1) - 31).toLocaleString("en", {
              minimumIntegerDigits: 2,
            })),
          (r += String.fromCharCode(parseInt(n, 10))))
        : (r += e.charAt(o));
  return btoa(unescape(encodeURIComponent(r)));
}
function decompressst(e) {
  var t,
    n,
    r,
    o,
  ...
  for (var s = 0; s < e.length; s++)
    (t = e.charCodeAt(s)) > 132
      ? ((n = t.toString(10)),
        (r = parseInt(n.substring(0, n.length - 2), 10)),
        (o = parseInt(n.substring(n.length - 2, n.length), 10) + 31),
        (a += String.fromCharCode(r) + String.fromCharCode(o)))
      : (a += e.charAt(s));
  return a;
}

function icompress(e) {
  e = unescape(encodeURIComponent(e));
  for (var n, t, r = "", o = 0; o < e.length; o += 2)
    (n = e.charCodeAt(o)),
      o + 1 < e.length
        ? ((t =
            n +
          ...
          (r += String.fromCharCode(parseInt(t, 10))))
        : (r += e.charAt(o));
  return btoa(unescape(encodeURIComponent(r)));
}
function idecompress(e) {
  var n,
    t,
    r,
    o,
  ...
  for (var c = 0; c < e.length; c++)
    (n = e.charCodeAt(c)) > 132
      ? ((t = n.toString(10)),
        (r = parseInt(t.substring(0, t.length - 2), 10)),
        (o = parseInt(t.substring(t.length - 2, t.length), 10) + 31),
        (a += String.fromCharCode(r) + String.fromCharCode(o)))
      : (a += e.charAt(c));
  return a;
}
...
function hide(e, t) {
  t || (t = document.documentElement),
    null == e && (e = !0),
    e ? t.setAttribute("hidden", "") : t.removeAttribute("hidden");
}
...
function unfade(a, k) {
  ...
  var l = setInterval(function () {
    t >= 1 && clearInterval(l),
      (a.style.opacity = t),
      (a.style.filter = "alpha(opacity=" + 100 * t + ")"),
      (t += 0.1 * t);
  }, k);
}
function fade(t) {
...
function notReady(ms) {
  ms ||
    (ms = `<!DOCTYPE html><head><style>html,body{font-size:1.25rem}</style></head><pre style="word-wrap: break-word; white-space: pre-wrap;">This page will be available once the Ladder Season is underway.</pre>`);
  {
    let tt = document.title;
    document.documentElement.innerHTML = ms;
    document.title = tt;
    hide(0);
    return false;
  }
...
function get_darkmode() {
  let isdarkmode = null;
  try {
    if (localStorage.getItem(brick("$_darkMode")) != null) {
      isdarkmode = JSON.parse(localStorage.getItem(brick("$_darkMode")));
    }
  } catch (ex) {
    console.error(ex);
  }
  if (location.href.indexOf("darkmode=0") > -1) {
    isdarkmode = false;
    try {
      localStorage.setItem(brick("$_darkMode"), false);
    } catch (ex) {}
  }
  if (location.href.indexOf("darkmode=1") > -1) {
    isdarkmode = true;
    try {
      localStorage.setItem(brick("$_darkMode"), true);
    } catch (ex) {}
  }
  if (isdarkmode == null) {
    if (
...
function sendBug(b) {
  ...
  try {
    ...
    return `${test.href}?501`;
  }
}

String.prototype.between = function (a, b) {
  try {
    return this.split(a)[1].split(b)[0];
  } catch (ex) {
    return this;
  }
};

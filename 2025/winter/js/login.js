...
document.addEventListener("DOMContentLoaded",function() {
    try {
        try{
            ...
            if(srch.indexOf('email=')>-1){byId('email').value=decodeURI(srch.split('email=')[1].split('&')[0]).replace('+'+ladderSeason.toLowerCase(), '')}
            if(srch.indexOf('key=')>-1){let ais = decodeURI(srch.split('key=')[1].split('&auto')[0].split('&return')[0]); try{if(LZString.decompressFromBase64(ais)){ais=LZString.decompressFromBase64(ais)}}catch{}; let ase = ais; byId('key').value=ase}
...
function authorize(d) {
  ...
  signIn({email: parsedEmail, key: d.key})
  .then((r)=>{
        if(r.error) {
            {try{hide(0)}catch(ex){}}
            document.querySelector('.modal.login').classList.remove('pulse')

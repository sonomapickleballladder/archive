...
function vrfid(tmd) {
    ...
    setTimeout(function(){saveData('availability2.0', injson).then(function(dn) {
        ...
        //showstatus(true);
        //cfstatus = true;
        let usrt = lsdciph(localStorage.getItem(brick('$_authenticated_user'+ladderSeason)),0,laddersOffered.Open.ls)
   ...
   try{
    ...
    if (parseInt(r.value.split(' ')[1]) == getRound()+1) {
       ...
       // byId('dates').innerHTML=`&nbsp;${offsetDate(parseInt((r.value.split(' ')[1]-1)*rdL))[1]} – ${offsetDate(parseInt((r.value.split(' ')[1]*rdL)-1))[1]}`;
        ...
        byId('rdlength').innerHTML = `&nbsp;<b>(${(Math.round(rdLs[parseInt(r.value.split(' ')[1])-1] * 2) / 2).toFixed(1)} Week Round${amef})</b>`;
}
    else{

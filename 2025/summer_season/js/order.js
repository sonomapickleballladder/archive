...
window.addEventListener('keydown', function keydownup(event){
  if( event.ctrlKey && (event.which ==40 || event.which ==39) ) {try{event.preventDefault(); byId('godown').click()}catch(ex){}}
  if( event.ctrlKey && (event.which ==37 || event.which ==38) ) {try{event.preventDefault(); byId('goup').click()}catch(ex){}}

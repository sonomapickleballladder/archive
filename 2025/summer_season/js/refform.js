...
function vrfrd(m) {
    ...
    if(prd<nrd) {
        event.preventDefault(); calert(false,'Error', 'You cannot offer to be a ref for a previous round.','error', 0,0,0,0,'1.5em'); return false;
    }
    if(prd>nrd+1) {
        event.preventDefault(); calert(false,'Error', 'You may only  offer to be a ref for the current or next Round. Please wait until '+byId('round').children[byId('round').selectedIndex-1].value+' to submit this form for '+byId('round').value+'.','error', 0,0,0,0,'1.5em'); return false;
    }
...
function postInfo() {
    ...
    name=name.replace(unreal,'').replace(unwntd,'').trim(); email = email.replace(unreal,'').replace(unwntd,'').trim(); phone = phone.replace(unreal,'').replace(unwntd,'').trim(); week1days=week1days.replace(unreal,'').replace(unwntd,'').trim();
    ...
    let injson = {round: round, name: name, email: email, phone: phone, week1days:week1days}
    ...
    try {
        saveData('refs',injson).then(function(res) {
            ...
            else { document.body.innerHTML = 'Success! You will be redirected back to the Refs Page momentarily.';copyData(injson); setTimeout(function() {location.replace( `refs${protoMap[location.protocol]}`)},3000)}
        })
    } catch (ex) {
...
function retainvals() {
    ...
    if(lscrds) {
        ...
        if(yg) {
            ...
            yg.email = yg.email.replace(`+_${ladderSeasonNameLowercase}_${ladderYear}`, "")

  ...
  ptc("viewable_Signups", "*", "awx", "1").then((res) => {
    ...
    for (let i = 0; i < ldo.length; i++) {
      ...
      let cdu = "",
        ...
        awc = "";
      try {
        if (laddersOffered[ldo[i]].min != 0) {
          star = "*";
          abo = "";
          awc = "line-through";
          cdu = ``; /*, at least ${parseInt(byId(`mns-${ldo[i]}`).innerHTML) - signupsObject[ldo[i]].length} still needed.*/
        }
      } catch (ex) {
        console.error(ex);
      }
      let sci = "";
      if (signupsObject[ldo[i]].length > 0) {
        sci = `&nbsp;&nbsp;<button class="cool_button hlfpd" onclick="showSignups('${ldo[i]}')"> View signups &nearr; </button>`;
      }
      try {
        byId(`ns-${ldo[i]}`).innerHTML =
          `${signupsObject[ldo[i]].length == 0 ? "None" : signupsObject[ldo[i]].length}${abo}${cdu}${sci}`;
      } catch (ex) {}
      try {
        byId(`ns-${ldo[i]}`)
          .closest("details")
          ...
          byId(`ns-${ldo[i]}`)
            ...
            .split("*")[0]
            .trim() +
          `${signupsObject[ldo[i]].length == 0 ? star : star + ' <span style="font-size:85%;text-shadow: 0px 1px 2px mediumseagreen;">[' + signupsObject[ldo[i]].length + "]</span>"}`;
      } catch (ex) {}
      try {
        byId(`ns-${ldo[i]}`)
          .closest("details")
          .querySelector("summary").style.textDecoration = awc;
      } catch (ex) {}
      if (i == ldo.length - 1) {
        console.log(signupsObject);
      }
    }
  });
}
...
function showSignups(l) {
  ...
  if (res.length > 0) {
    pepl = res.map(
      (a) => `<small><i>${l == "Team" ? a.team_name : a.name}</i></small><br>`,
    );
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (location.href.indexOf("ladder=") > -1) {
    try {
      let lad = location.href
        .split("ladder=")[1]
        .split("&")[0]
        .split("?")[0]
        .split("#")[0];
      let aa = byId(`ns-${lad}`).closest("details");
      aa.id = `${lad}_Ladder`;
      location.href = "#";
      location.href = `#${aa.id}`;
      aa.open = true;

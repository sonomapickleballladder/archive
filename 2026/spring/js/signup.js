...
function validate(c) {
  ...
  const not_required = ["phone", "invite"];
  let flds = document.querySelectorAll(`input[c]`);
  for (let ir = 0; ir < flds.length; ir++) {
    let anc1 = flds[ir].closest(".wrap");
    if (
      !anc1.classList.contains("h") &&
      not_required.includes(flds[ir].getAttribute("c")) == false
    ) {
      ...
      for (let i = matchks.length - 1; i > -1; i--) {
        if (!ks.includes(matchks[i])) {
          ...
          for (let ib = 0; ib < qs.length; ib++) {
            ...
            anc.classList.add("awaiting");
            qs[ib].placeholder = "Required";
            qs[ib].addEventListener("input", () => {
              anc.classList.remove("awaiting");
              if (qs[ib].getAttribute("dplaceholder")) {
                qs[ib].placeholder = qs[ib].getAttribute("dplaceholder");
              } else {
                qs[ib].placeholder = "";
              }
            });
        ...
        if (i == 0) {
          ...
          if (!sjenp.classList.contains("h") && !sjen.checked) {
            sjenp.classList.add("awaiting");
            sjen.focus();
            return false;
          } else {
            sjenp.classList.remove("awaiting");
          }
        }
        if (i == 0 && lsm < 1) {
          try {
          ...
          try {
            delete c.rating_type;
          } catch (ex) {}
          try {
            c.first_name = c.first_name.proper(1);
          } catch (ex) {}
          try {
            c.last_name = c.last_name.proper(1);
          } catch (ex) {}
          try {
            if (byId("display_name").value.trim()) {
              c.display_name = byId("display_name").value.trim();
            } else {
          ...
          try {
            if (
              document
                .querySelector("input[c=fallback_ladder]")
                .closest(".wrap")
                .classList.contains("h")
            ) {
              delete c.fallback_ladder;
            }
          } catch (ex) {}
          try {
            c.darktheme =
              [...document.body.classList].indexOf("dark_theme") > -1;
          } catch (ex) {}
          try {
            if (c.partner_name) {
              let vd = [];
              vd.push(c.display_name);
              vd.push(c.partner_name);
              vd = vd.sort((a, b) => a.localeCompare(b)).join(" & ");
              c.team_name = vd;
            }
          } catch (ex) {}
          console.log(c);
          ...
          if (
            !confirm(
              "Do you wish to proceed? You will not be able to go back and edit your responses once submitted (although you can ask me [the administrator] to change them).",
            )
          ) {
            unpulse();
            return false;
          ...
          register(c).then(function (res) {
            ...
            if (!res.error) {
              try {
                c.secret_key = LZString.compressToBase64(c.secret_key);
              } catch (ex) {}
              saveData("Signups", c).then(function (response) {
                // console.log(response);

                for (let ise = 0; ise < c.ladder.length; ise++) {
                  let lder = c.ladder[ise];
                  try {
                    let aivx = c.display_name;
                    if (!byId("vbconsent").checked) {
                      aivx = "anonymous";
                    }
                    let avol = {
                      name: aivx,
                ...
                if (!response.error) {
                  try {
                    snEm({
                      email: ["SPLinfo+newsignup@riseup.net"],
                      subject: "New Signup",
                      bod: `Hi there,%0AJust wanted to let you know that ${c.first_name} ${c.last_name} just signed up for ${c.ladder}%0AHave a nice day!%0ABot`,
                    });
                  } catch (ex) {
                    console.error(ex);
                  }
                  localStorage.setItem(
                    brick("$_hasRegistered" + ladderSeason),
                    true,
                  );
                  localStorage.setItem(
                    brick("$_inviteCode" + ladderSeason),
                    c.invite_code,
                  );
                  document.body.innerHTML = `
						<p>Successfully submitted!</p>
						  <p>You should receive a confirmation email shortly.
						  <a href="javascript:void(0)" onclick="noConfirmationEmail()">Didn't get one?</a></p>
						  <p>Once you have confirmed your email and paid the registration fee, you are all set for now. I will send out more information ${JSON.parse(getRound()) > 0 ? "before the start of the next round" : " after the initial registration deadline. In the meantime, please encourage as many of your friends and similarly-skilled players as you can to sign up as well. The more players are involved, the better the experience will be for you and for everyone else. You will find an invite code when returning to the Signup page (this will also be in your Dashboard) which you can use to invite players and receive a significantly discounted rate yourself. "}</p>
						  <p><a href='signup.html#register'>&larr; Proceed to pay Registration Fee</a></p>`;
              ...
              unpulse();
              alert(
                "Registration failed. Please make sure you have provided a valid email address, and a password that is at least 6 characters long.",
              );
            }
          });
        }
...
function toggleFallback(f) {
  ...
  try {
    byId("prefer_team").checked
      ? byId("partner_field").classList.remove("h")
      : byId("partner_field").classList.add("h");
  } catch (ex) {
    console.log(ex);
  }
}
...
window.onload = () => {
  ...
  try {
    byId("passct").style.width =
      `calc(100% + ${getComputedStyle(byId("vbtoggle")).width})`;
  ...
  try {
    ...
    if (isSafari) {
      let cb = document.querySelectorAll(
  ...
  try {
    ...
    let svdin = lsdciph(
      localStorage.getItem(brick("$_signupFormInput" + ladderSeason)),
    );
    console.log(svdin);
    if (hasreg) {
      if (
        location.hash.indexOf("view-responses") < 0 &&
        location.href.indexOf("force=") < 0
      ) {
        document.body.innerHTML =
          'You have already signed up. Click <a href="?reload=1#view-responses">here</a> to view your responses.';
      } else if (location.href.indexOf("force=") < 0) {
        if (!svdin) {
    ...
    for (let i = 0; i < sins.length; i++) {
      ...
      if (svn.type == "radio") {
        if (typeof val == "number") {
          [...svn.closest(".wrap").querySelectorAll(`input[c=${sins[i]}]`)][
            val
          ].checked = "true";
        }
        ...
        for (let ix = 0; ix < sune.length; ix++) {
          let sej = [
            ...sune[ix]
              .closest(".wrap")
              .querySelectorAll(`input[c=${sins[i]}]`),
          ].indexOf(sune[ix]);
          if (sej > -1) {
            if (val[sej] == true) {
              sune[ix].checked = true;
            }
          }
        }
      } else svn.value = val;
  ...
  try {
    if (location.href.indexOf("invite=") > -1) {
      let invite = location.href
        .split("?invite=")[1]
        .split("&")[0]
        .split("?")[0];
      byId("invite").value = invite;
    }
  } catch (ex) {}
...
function saveFormInput() {
  try {
    if (
      location.href.indexOf("&save=no") < 0 ||
      location.href.indexOf("?save=no") < 0
    ) {
      let sn = document.querySelectorAll("input[c]"),
        kaim = {};
      ...
      for (let i = 0; i < sn.length; i++) {
        ...
        if (sn[i].type == "radio") {
          if (sn[i].checked)
            kaim[ah] = [
              ...sn[i].closest(".wrap").querySelectorAll(`input[c=${ah}]`),
            ].indexOf(sn[i]);
        } else if (sn[i].type == "checkbox") {
          let snei = document.querySelectorAll(
            `input[c=${sn[i].getAttribute("c")}]`,
          );
        ...
        if (i == sn.length - 1 && location.hash.indexOf("view-responses") < 0) {
          ...
          localStorage.setItem(
            brick("$_signupFormInput" + ladderSeason),
            lsciph(kaim),
          );
        }
      }
...
function suggestPartner(inp) {
  try {
    let bya =
      `${document.querySelector("input[c=first_name]").value}${document.querySelector("input[c=last_name]").value}`
        .replace(/\s/g, "")
        .toLowerCase();
    let nhcx = signupsObject.Team.filter(
      (a) => a.partner_name.replace(/\s/g, "").toLowerCase() == bya,
    );
    let datl = byId("partner_list");
    datl.innerHTML = "";
...
window.addEventListener("keydown", (e) => {
  //if(e.ctrlKey && e.which == 13) {jsonify()}
});

function ischeckable(c) {
  try {
    return "radio" == c.type || "checkbox" == c.type;
  } catch (c) {
    return !1;
  }
}

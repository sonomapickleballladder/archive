  ...
  for (let i = 0; i < ud.length; i++) {
    ptc("subs", "*", "round", getRound() ? getRound() : 1).then(function (res) {
      ...
      try {
        ...
        byAttr("sublist")[0].innerHTML = `<p>No one yet...</p>`;
      }
    });
  }
}
function displaySubs(sb) {
  ...
  for (let i = 0; i < sb.length; i++) {
    let name = sb[i].name,
      ...
      phone = sb[i].phone,
      skill = sb[i].skill,
      in_ladder = sb[i].in_ladder,
      already_playing = sb[i].already_playing,
      week1days = sb[i].week1days,
      week2days = sb[i].week2days;
    ...
    if (in_ladder == false) {
      judge = "psx";
    } else if (already_playing == true) {
      judge = "psy";
    } else if (already_playing == false) {
      judge = "psz";
    }
    byAttr("sublist")[0].innerHTML +=
      `<details class="${judge}"><summary>${name}</summary><p class="infop">Email: <a href="mailto:${email}">${email}</a></p>

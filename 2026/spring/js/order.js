  ...
  ptc("rankings", "*").then(function (res) {
    ...
    try {
      ...
      byId("order").innerHTML =
        `<br><span style='opacity: 0'>Initial Order Creation will be initialized 3 days before the first round starts.</span><br>`;
...
function renderorder(al, ord) {
  byId("order").innerHTML = "";
  try {
    byId("goup").removeAttribute("id");
    byId("godown").removeAttribute("id");
  } catch (ex) {}
  ...
  for (let i = 0; i < ord.length; i++) {
    ...
    ord[i] = ord[i].split(":")[0];
    let controls = "";
    if (name == ord[i].toLowerCase().replace(/(\s)/g, "") && checkid()[1]) {
      controls = `&nbsp;<button class="cool_button" id="goup" onclick="moveUp(this.parentNode,${i + 1},${orind + 1})">Up &uarr;</button>&nbsp;<button class="cool_button" id="godown" onclick="moveDown(this.parentNode,${i + 1},${orind + 1})">Down &darr;</button>`;
      byId("order").innerHTML +=
        `<li><span class="active">${ord[i]}</span>${controls}</li>`;
    } else {
...
window.addEventListener("keydown", function keydownup(event) {
  if (event.ctrlKey && (event.which == 40 || event.which == 39)) {
    try {
      event.preventDefault();
      byId("godown").click();
    } catch (ex) {}
  }
  if (event.ctrlKey && (event.which == 37 || event.which == 38)) {
    try {
      event.preventDefault();
      byId("goup").click();
    } catch (ex) {}
...
function moveDown(element, p1, p0) {
  if (element.nextElementSibling) {
    let vert =
      Array.from(element.parentNode.children).indexOf(element) - p0 + 2;
    let indchange = Math.abs(vert);
...
async function ptc(t, e, c, n, a, i, r) {
  const { data: o, error: u } = await sb.client
    .from(init.f_e + ladderId.sb + t)
    .select(e);
  u && debug;
  try {
    executeFunctionByName(r, window, o);
  } catch (t) {
    return o;
  }
}
...
function checkid() {
  ...
  try {
    if (!lsdciph(localStorage.getItem(ladderId.ls + "ArprO_lA3RZxD5z"))) {
      ...
      let hjson = lsdciph(
        localStorage.getItem(ladderId.ls + "ArprO_lA3RZxD5z"),
      );
...
function savethisorder() {
  ...
  if (norder == wbicewom.order) {
    calert(0, "Error", "Order has not changed", 0);
    return false;
  }
  ...
  let smogp = {
    order: norder,
    Fz_id: checkid()[2].kwm,
    mwbn: LZString.compressToUTF16(hjson.name),
  };
...
function colorcodeorder() {
  ...
  for (let i = 0; i < amk.length; i++) {
    ...
    let active = namespan.classList.contains("active");

    if (
      bnsuawe.indexOf(lzmn) > -1 ||
      (hjson.name.toLowerCase().replace(/(\s)/g, "") ==
        nm.toLowerCase().replace(/(\s)/g, "") &&
        checkid()[1])
    ) {
      let indchange = Math.abs(vert);
      if (vert > 0) {
        indchange = Math.abs(vert + 2);
      }
      let inp = document.createElement("input");
      //console.log(vert);
      if (active) namespan.style.fontWeight = "bold";
      else if (vert < 0) namespan.style.backgroundColor = clrsup[indchange];
      else if (vert > 0) namespan.style.backgroundColor = clrsdown[indchange];
      else if (vert == 0) namespan.style.backgroundColor = "";
    }
    ...
    if (invrt.indexOf("-") > -1) {
      invrt = invrt.replace(/-/g, "") + "⇓";
    } else if (invrt.length > 0) {
      invrt = invrt + "⇑";
    }

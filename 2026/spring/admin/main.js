window.onload = function () {
  ...
  try {
    if (JSON.parse(localStorage.getItem(ladderSeason + "toggleautorefresh"))) {
      ...
      try {
        displaySignups(
          JSON.parse(localStorage.getItem(ladderSeason + "playerJSON28")),
          {
            store: JSON.parse(
              localStorage.getItem(ladderSeason + "playerJSON28"),
            ),
          },
        );
      } catch (ex) {
  ...
  try {
    if (adsb.client) {
      ...
      if (
        cipher("xsapevvue3g(xm")(prompt("Password")) ==
        "616b637176317c3e52314830"
      ) {
        hide(0);
      } else {
        setTimeout(function () {
          document.write(
            "<style>html,body{font-size:1.25rem}</style>unauthorized",
          );
        }, 200);
      }
    }
...
async function ptc(t, s, e, m, x, z, thenfunction) {
  const { data, error } = await adsb.client.from(init.f_e + t).select(s);
...
function savekey(k) {
  if (k.length < 1) {
    ...
    localStorage.setItem(ladderSeason + "admkey", LZString.compress(k));
  }
...
function getavlb(tb, ol, lad) {
  ...
  for (let ih = 0; ih < mp.length; ih++) {
    ...
    // console.log(nw.sb);
    let gwr = JSON.parse(tempStore.lsnups)[mp[ih]].signups.map((a) => {
      return {
        name: a.first_name + " " + a.last_name,
        expires: LZString.compressToEncodedURIComponent(
          LZString.decompressFromBase64(a.secret_key),
        ),
      };
    });
...
function dspavlb(ol, uij, kwm) {
  ...
  for (let i = 0; i < rarr.length; i++) {
    // console.log(rarr[i]);
    let dl = "\n";
    if (byId("delimit").value.length > 0) {
      dl = byId("delimit").value;
    }
    if (Boolean(rarr[i].is_available) == uij) {
      mah.value += rarr[i].name.trim() + dl;
    }
  }

  uniq.value = mah.value
    .split("\n")
  ...
  byId("count").innerHTML =
    JSON.stringify(uniq.value.split("\n").length - 1) +
    " / " +
    JSON.stringify(rarr.length);
  byId("ngroups").innerHTML = Math.ceil(
    (uniq.value.split("\n").length - 1) / 4,
  );
...
function displayNotAvailable(round, property, ladder) {
  ...
  const ladderJson = JSON.parse(
    localStorage.getItem(ladderSeason + "Ladders_Offered_Computation"),
  )[ladder].signups;
  const everyone = ladderJson.map((a) => `${a.first_name} ${a.last_name}`);
  const absentPlayers = everyone.filter(
    (player) => !available.includes(player),
  );
  ...
  for (let i = 0; i < absentPlayers.length; i++) {
    ...
    const email = ladderJson.filter(
      (player) => `${player.first_name} ${player.last_name}` == element,
    )[0].email;
    // console.log(email)
...
function getAlphabeticalList(ladder, element) {
  ...
  let alphabeticalList = ladderJson.map(
    (player) => `${player.first_name} ${player.last_name}`,
  );
  alphabeticalList = alphabeticalList.sort().join("\n");
  console.log(
    `Alphabetical List of Players for ${ladder} Ladder:\n\n${alphabeticalList}`,
  );
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      for (let ij = 0; ij < Object.values(res[i]).length; ij++) {
        ...
        if (
          byId("cvtime").checked &&
          yk == "created_at" &&
          Boolean(new Date(ky) != "Invalid Date")
        ) {
...
function getPlayerJSON() {
  ...
  let ms = "Signups";
  if (tb.trim().length < 1 || tb.value == ms) {
    byId("tablename").value = ms;
    getPlayerJSON();
    return false;
  } else {
    byId("svew").innerHTML +=
      '<span id="vload">&nbsp;<div style="display: inline-block; " class="loader"></div></span>';
    //console.log(tb)
    ptc(tb, "*").then((res) => {
      res = res.data;
      if (res) {
            ...
            for (let yi = 0; yi < lo.length; yi++) {
              ...
              if (typeof nh.ladder == "array" || typeof nh.ladder == "object") {
                if (nh.ladder.includes(bv)) {
                  ...
                  laddersOffered[bv].signups = laddersOffered[
                    bv
                  ].signups.reduce(function (a, b) {
        ...
        console.log(
          `%c${Date.now() - execTime["gpl"]}ms:%O`,
          "color: #64d462 ;",
          k,
        );
        let sntoday = k.responses.filter(
          (ah) =>
            new Date(ah.created_at).toString().split("2026 ")[0] ==
            new Date().toString().split("2026 ")[0],
        ).length;
        ...
        for (let i = 0; i < k.responses.length; i++) {
          ...
          if (gr.ladder.includes("Open")) {
            nx++;
          }
          if (gr.ladder.includes("Womens")) {
            nx++;
          }
          if (
            !gr.ladder.includes("Womens") &&
            !gr.ladder.includes("Open") &&
            gr.fallback_ladder == "Yes"
          ) {
            msk.push(gr);
          }
        }
        ...
        localStorage.setItem(
          ladderSeason + `Ladders_Offered_Computation`,
          JSON.stringify(laddersOffered),
        );
      }
    });
  }
...
function jsFill() {
  try {
    let hy = byAttr("jsfill");
    for (let i = 0; i < hy.length; i++) {
      let jf;
      jf = Boolean(hy[i].getAttribute("jsfill"))
        ? hy[i].getAttribute("jsfill")
        : "";
      try {
        typ = jf.split("?")[1];
        jf = jf.split("?")[0];
...
function displaySignups(s, mv) {
  ...
  byId("signups").innerHTML +=
    `<tr><th>Ladder Name</th><th>Number of Signups</th><th>Signups Still Required</th><th>New Signups</th><th>All Signups</th><th>Signups Today</th></tr>`;
  ...
  for (let i = 0; i < g.length; i++) {
    ...
    let tdsnps = s[bv].signups.filter(
      (ah) =>
        new Date(ah.created_at).toString().split("2026 ")[0] ==
        new Date().toString().split("2026 ")[0],
    ).length;
    if (s[bv].min == undefined) {
      s[bv].min = minSignupNumber;
    }
    let need = s[bv].min - ns;
    if (need < 1) need = "#/NA";
    let last = 0;
    try {
      if (localStorage.getItem(ladderSeason + "Ladder_Signups")) {
        last = JSON.parse(
          localStorage.getItem(ladderSeason + "Ladder_Signups"),
        )[bv].signups.length;
      }
    } catch (ex) {}
    let sem = s[bv].signups.map(
      (a) =>
        `<li id=${a.email} style=color:blue;cursor:pointer;text-decoration:underline; onclick=showIndPlayer(this)>${a.first_name} ${a.last_name}</li>`,
    );
    sem = sem.toString().replace(/(,)/g, "").replace(/(')/g, "‘");

    byId("signups").innerHTML +=
      `<tr id="${bv}"><td>${bv}</td><td>${ns}</td><td>${need}</td><td>${ns - last} New</td><td><a href="javascript:;" onclick="calert(0,'${bv} Ladder Signups', '<ol>${sem}</ol>', 'notice', 0,0,0,0,'1em','full', 'padding: 1em')">View &nearr;</a></td><td>${tdsnps}</td></tr>`;
    ...
    if (i == g.length - 1) {
      hide(0, byId("signups"));
      try {
        byId("svew").removeChild(byId("vload"));
      } catch (ex) {
        console.error(ex);
      }
      try {
        if (allnew > 0) {
          byId("svew").classList.add("newalert");
          byId("approvePlayers").classList.add("blink_me");
        }
      } catch (ex) {}
      if (mv.store) {
...
function showIndPlayer(p) {
  ...
  //console.log(omp);
  let ind = omp.filter((a) => {
    return a.email == p.id;
  })[0];
  //console.log(ind);
  let sjik = ind.secret_key;
  try {
    if (LZString.decompressFromBase64(sjik)) {
      sjik = LZString.decompressFromBase64(sjik);
    }
  } catch (ex) {}
  ...
  let cpy =
    '<button onclick="navigator.clipboard.writeText(this.previousElementSibling.href)">&nbsp;Copy&nbsp;</button>';
  ...
  child.document.write(`<html>
    ...
    <div>
    <p><a href="javascript:;" onclick="navigator.clipboard.writeText('${ind.email}')">Copy email (${ind.email})</a></p>
    </div>
   </details> </p>
   ...
   <p> <details>
     ...
     <p><a target="_blank" href="${getPFUrl(2)}login${protoMap[location.protocol]}?spl=${lsciph("email=" + ind.email + "&key=" + LZString.compressToBase64(sjik) + "&autologin", false, laddersOffered["Womens"].ls)}&return_to=${getPFUrl(2)}@Womens/confirm_availability${protoMap[location.protocol]}">Confirm ${ind.first_name}'s Availability Womens &nearr;</a> ${cpy}</p>
     ...
     <p><a target="_blank" href="${getPFUrl(2)}login${protoMap[location.protocol]}?spl=${lsciph("email=" + ind.email + "&key=" + LZString.compressToBase64(sjik) + "&autologin", false, laddersOffered["Womens"].ls)}&return_to=${getPFUrl(2)}@Womens/initial_order${protoMap[location.protocol]}">Test Initial Order Womens &nearr;</a> ${cpy}</p>
    ...
    <p> <details>
     ...
     <p><a target="_blank" href="https://sonomapickleballladder.github.io/${ladderSeasonNameLowercase}_season/login?spl=${lsciph("email=" + ind.email + "&key=" + LZString.compressToBase64(sjik) + "&autologin", false, laddersOffered["Womens"].ls)}&return_to=/${ladderSeasonNameLowercase}_season/@Womens/confirm_availability${protoMap[location.protocol]}">Confirm ${ind.first_name}'s Availability Womens &nearr;</a> ${cpy}</p>
     ...
     <p><a target="_blank" href="https://sonomapickleballladder.github.io/${ladderSeasonNameLowercase}_season/login?spl=${lsciph("email=" + ind.email + "&key=" + LZString.compressToBase64(sjik) + "&autologin", false, laddersOffered["Womens"].ls)}&return_to=/${ladderSeasonNameLowercase}_season/@Womens/initial_order${protoMap[location.protocol]}">Test Initial Order Womens &nearr;</a> ${cpy}</p>
...
function addPayOpt(e) {
  if (
    byId("ptdate").value.length < 1 ||
    byId("ptend").value.length < 1 ||
    byId("ptstart").value.length < 1 ||
    byId("ptloc").value.length < 1 ||
    byId("ptpers").value.length < 1
  )
    return false;
  console.log(e);
  ...
  console.log(fb);
  var seconds =
    Math.round(fb.getTime() / 1000) +
    parseInt(byId("ptstart").value.split(":")[0]);
  let conc = {
    0: "th",
    1: "st",
    2: "nd",
  ...
  let dat =
    sne.days[fb.getDay()] +
    " " +
    sne.months[fb.getMonth() + 1] +
    " " +
    fb.getDate() +
    conc[fb.getDate()];
  //console.log(dat);
  let stm =
    tConvert(byId("ptstart").value) + " – " + tConvert(byId("ptend").value);
  let jsonob = {
    id: seconds,
    time: stm,
    location: byId("ptloc").value,
    date: dat,
    person: byId("ptpers").value,
    awvnu: "1",
  };
  console.log(jsonob);
...
window.addEventListener("beforeunload", function (e) {
  try {
    if (
      byId("markasseen").checked &&
      Object.keys(JSON.parse(tempStore.lsnups)).length > 0
    ) {
      localStorage.setItem(ladderSeason + "Ladder_Signups", tempStore.lsnups);
    }
  } catch (ex) {}

  try {
    localStorage.setItem(
      ladderSeason + "lastview",
      JSON.stringify(
        [...document.querySelectorAll("details")].map((e) => e.open),
      ),
    );
  } catch (ex) {}
});
...
async function adsaveData(t, e, thenfunction) {
  const { data, error } = await adsb.client.from(init.f_e + t).insert([e]);

  if (error) console.log(error);

  try {
    executeFunctionByName(thenfunction, window, data);
  } catch (ex) {}
...
function idwh(k) {
  for (let i = 0; i < k.responses.length; i++) {
    ...
    let fn = eg.first_name.trim(),
      ln = eg.last_name.trim(),
      fln = `${fn} ${ln}`,
      pw = LZString.decompressFromBase64(eg.secret_key);
    afp[fln] = pw;
    eml[fln] = eg.email;
    fp.push(fln);
  }
...
function confirmsubmit(k, s) {
  let ty = confirm(
    `Are you sure you want to confirm availability for ${k} for round ${s}?`,
  );
  if (!ty) {
    return false;
  }
...
function confirmupdate(k, s) {
  let ty = confirm(
    `Are you sure you want to update availability for ${k} for round ${s}?`,
  );
  if (!ty) {
    return false;
  }
...
function confirmavbl(k, s) {
  ...
  let lm = {
    name: k,
    token: sha256x(st, 1),
    expires: LZString.compress(afp[k]),
  };
...
function checkstatus() {
  ...
  ptc("availability2.0", "encoding3", "smvig", kp.token).then(function (res) {
    ...
    if (res && res.length > 0) {
      ...
      let rsje;
      try {
        rsje = parseInt(byId("cfround").value);
      } catch (ex) {
        rsje = getRound() + 1;
      }
      //console.log(rsje);
      ...
      for (let i = 0; i < res.length; i++) {
        ...
        //console.log(fa);
        if (parseInt(fa.split("/")[0].trim()) === rsje) {
          acwom = res[i].encoding3;
          return false;
        }
        if (i == res.length - 1) {
          return false;
...
function vrfid(tmd, s) {
  ...
  console.log(injson);
  let kf = confirm(
    `By clicking okay, you are confirming that you (${bw.name}) are available for ${byId("round").value}.\r\n \r\nIf you are not, please hit cancel.`,
  );
  ...
  setTimeout(function () {
    saveData("availability2.0", injson).then(function (dn) {
      if (!dn) {
        ...
        calert(
          0,
          "Server Error",
          "Could not proccess your request. Please confirm by email instead.",
        );
      }
    });
  }, 500);
...
function unvrfid(tmd, s) {
  ...
  if (!tmd) {
    mgea = `By clicking okay, you (${bw.name}) are withdrawing your availability for round ${byId("cfround").value}.\r\n \r\nIf you do not wish to proceed, please hit cancel.`;
  }
  let kf = confirm(mgea);
  ...
  setTimeout(function () {
    declareData("availability2.0", injson, "encoding3", ocgso).then(
      function (dn) {
        console.log(dn);
        if (!dn) {
          getavlb("availability2.0", s);
...
function checkplayers(r, ld) {
  ...
  ptc(laddersOffered[ld].sb + "availability2.0", "*").then((res) => {
    ...
    let fdc = JSON.parse(tempStore.lsnups)[ld].signups.map((a) => {
      return {
        name: a.first_name + " " + a.last_name,
        expires: LZString.compressToEncodedURIComponent(
          LZString.decompressFromBase64(a.secret_key),
    ...
    for (let er = 0; er < fdc.length; er++) {
      ...
      let hv = res.filter((e) => {
        return (
          ...
          kp.name.replace(/(\s)/g, "").toLowerCase()
        );
      });
      //console.log(hv);
      ...
      for (let lu = 0; lu < hv.length; lu++) {
        let dcip = decipher(
          LZString.decompressFromEncodedURIComponent(kp.expires),
        );

        try {
          let fa = dcip(hv[lu].encoding3);
          //console.log(parseInt(fa.split('/')[0].trim())===getRound()+1);
          //console.log(fa);
          if (
            (parseInt(fa.split("/")[0].trim()) == r ||
              parseInt(hv[lu].round) == fa ||
              (hv[lu].smvig == "4873" && parseInt(hv[lu].round) == r)) &&
            hv[lu].is_available
          ) {
            nd.push(hv[lu].name); /* console.log(res[i].name);*/
          }
        } catch (ex) {
          console.error(ex);
        }

window.onload = function () {
  ...
  if (!localStorage.getItem(ladderSeason + "Ladders_Offered_Computation")) {
...
function addplayers(val) {
  ...
  if (!byId("check").checked) {
    val = localStorage.getItem(ladderSeason + "Ladders_Offered_Computation");
  }
  ...
  for (let mni = 0; mni < Object.keys(h).length; mni++) {
    ...
    for (let i = 0; i < bh.length; i++) {
      ...
      try {
        if (LZString.decompressFromBase64(skey)) {
          skey = LZString.decompressFromBase64(skey);
        }
      } catch (ex) {}
      let cmbin = nam + skey;
      ...
      if (nhf.includes(ld)) {
        ...
        let contact = {
          name: `${bh[i].first_name} ${bh[i].last_name}`,
          email: bh[i].email,
          phone: bh[i].phone,
          prefer: bh[i].prefer,
          common: 1,
        ...
        console.log(contact);
        adsaveData("contact", contact, ldv[ld].sb);
      }
...
function spreadsheetify() {
  ...
  al.onclick = function () {
    window.open(
      "https://calc.domainepublic.net/ninnisonue93n9mMNDicw9csd9ckMwc",
      "_blank",
    );
  };
...
async function addelete(t) {
  ...
  const { data, error } = await adsb.client.from(init.f_e + t).delete();
}

function updatePaymentTable(boolean) {
  let ldv = { Open: { sb: "" }, Womens: { sb: "womens-ladder_" } };
  let ladder = prompt("Which ladder is this for?");
  notPaidArray = byId(Boolean(boolean) ? "paid" : "notpaid")
    .value.trim()
    .split("\n");

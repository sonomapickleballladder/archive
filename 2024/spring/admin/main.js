  ...
  if(tb.trim().length<1 || tb.value == ms) {byId('tablename').value = ms; getPlayerJSON(); return false}
    ...
    .then ((res) => {
      ...
      if(res) {
        ...
        let sntoday = k.responses.filter(ah => new Date(ah.created_at).toString().split('2024 ')[0] == new Date().toString().split('2024 ')[0]).length;
...
function displaySignups(s, mv) {
  ...
  for (let i = 0; i < g.length; i++) {
    ...
    let tdsnps = s[bv].signups.filter(ah => new Date(ah.created_at).toString().split('2024 ')[0] == new Date().toString().split('2024 ')[0]).length
